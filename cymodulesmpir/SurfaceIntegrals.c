...
double Integrate_ellipsoid_surface(double alpha, void *prms){
  ...
  if (nvertices_B > 0){
    ...
    r[1] = newy;
    r[2] = newz;
    int trigger[1] = {(params->trigger[0])}; 
    ...
    datastuff[7] = zc;
    datastuff[14] = -1;
    phi = (prefac/2)*potential_polyhedron_mpir(r, vertices_in, nfaces_B, face_ids_other, trigger, datastuff, Rot_A, Rot_B_T);
    params->trigger[0] = trigger[0];
...
double Integrate_tetrahedron_surface(double v, void *prms){
  ...
  */
  ...
  if (faces_B > 0){
    ...
    datastuff[14] = 1;
    /*
    This trigger variable is needed to 'trick' the integrator near points where the potential is imaginary/singular.
    Because the integrator is adaptive, a change in number can cause it to diverge, as the function is no longer 'smooth'.
    The trigger ensures that this does not happen.
    */
    int trigger[1] = {(params->trigger[0])}; 
    phi = (prefac/2)*potential_polyhedron_mpir(r, vertices_in, faces_B, face_ids_other, trigger, datastuff, Rot_A, Rot_B_T);
    params->trigger[0] = trigger[0];
    for (i=0; i<nvertices_B; i++){
      free(vertices_in[i]);
    }
    ...
    //double phi = grav_potential_face(r, r_i, n_i, vertices, prefac);
    // Compute potential of ellipsoid
    double ka;
    if (fabs(a_B - b_B) < 1e-15){
      ka = kappa_value(a_sq, c_sq, x1_sq, y1_sq, z1_sq);
...
int Force_point_mass(double input[11], double semiaxes[6], int eulerparam, double **vertices, int vertexids[7],
  ...
  */
  ...
  if (nfaces_B > 0){
    ...
    datastuff[14] = -1;
    datastuff[15] = -1;
    double phi = (prefac/2)*grav_field_polyhedron_mpir(r, vertices_in, nfaces_B,
                       face_ids_other, g_field, trigger, datastuff, Rot_B, Rot_B_T, 1);
...
double integrate_potint_ellipsoid_surface(double t, void *p){
  ...
  */
  ...
  if (nvertices_B > 0){
    ...
    datastuff[14] = -1;
    datastuff[15] = -1;
    phi = (rho_B/2)*grav_field_polyhedron_mpir(r, vertices_in, nfaces_B,
                       face_ids_other, g_field, trigger, datastuff, Rot_A, Rot_B_T, 1);
    gx_n = -g_field[0];
    gy_n = -g_field[1];
    gz_n = -g_field[2];
    for (int i=0; i<nvertices_B; i++){
...
double integrate_potint_polyhedron_surface(double v, void *p){
  ...
  */
  ...
  int nvertices_A = (params->nvert_A);
  int nvertices_B = (params->nvert_B);
  int nfaces_A = (params->nfaces_A); 
  int nfaces_B = (params->nfaces_B);
  int bodyB_ID = (params->bodyB_ID);
  ...
  if (nvertices_B > 0){
    ...
    datastuff[14] = -1;
    datastuff[15] = -1;
    phi = (prefac/2)*grav_field_polyhedron_mpir(r, vertices_in, nfaces_B,
                       face_ids_other, g_field, trigger, datastuff, Rot_A, Rot_B_T, 0);
    gx_n = -g_field[0];
    gy_n = -g_field[1];
    gz_n = -g_field[2];
    for (int i=0; i<nvertices_B; i++){
...
double mutual_potential_point_mass(double semiaxes[6], double input_values[11], double *vertices1D,
  ...
  */
  ...
  if (nfaces_B > 0){
    ...
    datastuff[7] = zc;
    datastuff[14] = -1;
    double Rot_A[3][3] = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
    Epot = (prefac*rho_B/2)*potential_polyhedron_mpir(r, vertices_in, nfaces_B, face_ids_other, trigger, datastuff, Rot_A, Rot_B_T);
