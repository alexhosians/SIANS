...
from libc.math cimport pi,cos,sin,tan,sqrt,asin, asinh, abs, exp, acos
...
cdef inline double c_acos(double numerator, double denominator):
	if numerator == 0:
		return acos(0)
	else:
		return acos(numerator/denominator)

cdef inline double moment_inertia_spheroid_C(double a,double c,double rho,int i):
	""" 
	Values of the moment of inertia for spheroids 
	Returns a C-variable instead of python object
	"""
	cdef double M = (4.0/15.0)*pi*rho*a*a*c
...
cdef inline int kronecker_delta(int i, int j):
	if i == j:
		return 1
	else:
		return 0

cdef inline int moment_inertia_tetrahedron(double rho, double volume, double[:] vertices, double[:,:] I):
	"""
	Computes moment of inertia matrix of a tetrahedron.
	"""
	cdef double prefac = rho*volume/20
	cdef int i, j
	cdef double a = 0
	cdef double b = 0
	cdef double c = 0
	cdef double a_prime = 0
	cdef double b_prime = 0
	cdef double c_prime = 0
	cdef int i3, j3
	for i in range(4):
		i3 = 3*i
		for j in range(i+1):
			j3 = 3*j
			a += vertices[i3+1]*vertices[j3+1] + vertices[i3+2]*vertices[j3+2]
			b += vertices[i3]*vertices[j3] + vertices[i3+2]*vertices[j3]
			c += vertices[i3]*vertices[j3] + vertices[i3+1]*vertices[j3+1]
		for j in range(4):
			j3 = 3*j
			a_prime += (1 + kronecker_delta(i,j))*vertices[i3+1]*vertices[j3+2]
			b_prime += (1 + kronecker_delta(i,j))*vertices[i3]*vertices[j3+2]
			c_prime += (1 + kronecker_delta(i,j))*vertices[i3]*vertices[j3+1]

	I[0][0] = 2*prefac*a
	I[0][1] = -prefac*b_prime
	I[0][2] = -prefac*c_prime
	I[1][0] = -prefac*b_prime
	I[1][1] = 2*prefac*b
	I[1][2] = -prefac*a_prime
	I[2][0] = -prefac*c_prime
	I[2][1] = -prefac*a_prime
	I[2][2] = 2*prefac*c
	return 0
...
cdef inline double matrix_multiplication(double[:,:] R1, double[:,:] R2, double[:,:] R):
	""" 
	Function to performs matrix multiplication R = R1*R2 
	The matrices are assumed to be square-matrices
	"""
	assert R1.shape[0] == R2.shape[0]
	assert R1.shape[1] == R2.shape[1]
	assert R1.shape[0] == R2.shape[1]
	assert R1.shape[0] == R.shape[0]
	assert R1.shape[1] == R.shape[1]
	assert R1.shape[0] == R.shape[1]
	cdef int N_x = R1.shape[0]
	cdef double sum_val
