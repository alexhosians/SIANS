...
import cython
import sys
from libc.math cimport pi,cos,sin,tan,sqrt,asin,asinh,acos
from cython.parallel cimport prange, parallel
from scipy.integrate import simps, dblquad

@cython.boundscheck(False)
@cython.wraparound(False)
cpdef rotation_matrix(double phi,double theta,double psi):
	""" 
	...
	"""
	cdef double cosphi = cos(phi)
	cdef double sinphi = sin(phi)
	cdef double costheta = cos(theta)
	cdef double sintheta = sin(theta)
	cdef double cospsi = cos(psi)
	cdef double sinpsi = sin(psi)
	cdef double cospsisintheta = cospsi*sintheta
	cdef double sinpsisintheta = sinpsi*sintheta
	R = np.zeros((3,3))
...
cpdef Compute_energies_tetrahedron_2body(double[:,:] semiaxes, double[:] massdens, double[:,:] sol, int[:] int_info,
 double[:] tolerance_arrs, double[:] vertices1D, int[:] p_info, double G_grav, int[:] index_ordering):
	...
	Intended to work for two-body case with one ellipsoid and one tetrahedron.
	Body A is the tetrahedron.
	"""
	...
	for i in range(N_step):
		...
		#E_rotB = half*(I_11_B*omega_x[1][i]**2 + I_22_B*omega_y[1][i]**2 + I_33_B*omega_z[1][i]**2)
		vA = vx[0][i]**2 + vy[0][i]**2 + vz[0][i]**2
		vB = vx[1][i]**2 + vy[1][i]**2 + vz[1][i]**2
		U[i] = -Ep*mass_scale
		Ek[i] = half*(mA*vA + mB*vB)*mass_scale
		#Erot[i] = E_rotA + E_rotB
...
cpdef Compute_energies_Nbody(double[:,:] semiaxes, double[:] massdens, double[:,:] sol, int[:] int_info,
 double[:] tolerance_arrs, double[:] vertices1D, int[:] p_info, double G_grav, int[:] index_ordering):
	...
	cdef double[:] I_1D
	cdef double[:] vertices_tetra_in
	cdef int[:] face_ids_memview# = np.zeros(3*N_faces[1], dtype=np.int32)
...
cpdef getforce(double[:] pos, double[:] angles, double[:] massdens, double[:] semiaxes, int[:] nfaces, int[:] nverts,
	 double[:] vertices, int[:] face_ids, int FM_check, double G_grav, double eabs=1e-8, double erel=1e-8, int qval=6,
	 int roundoff=0):
	"""
	...
	Applicable for ellipsoids and polyhedron
